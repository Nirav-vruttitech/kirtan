...
import { setFontColorValue } from './../Slice/plateSlice';

const SketchExample1 = () => {
    const dispatch = useDispatch();
    const color = useSelector(state => state.viewPort.fontColorValue);
    ...
    useEffect(() => {
        if (fontColorState.color) {
            localStorage.setItem('fontColorValue', (fontColorState.color));
            dispatch(setFontColorValue(fontColorState.color));
        }
    }, [fontColorState]);

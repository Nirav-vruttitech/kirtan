...
import { setViewPortBgColor } from './../Slice/plateSlice';

const SketchExample2 = () => {
    const dispatch = useDispatch();
    const color = useSelector(state => state.viewPort.ViewPortBgColor);
    ...
    useEffect(() => {
        if (fontColorState.color) {
            localStorage.setItem('ViewPortBgColor', (fontColorState.color));
            dispatch(setViewPortBgColor(fontColorState.color));
        }
    }, [fontColorState]);
